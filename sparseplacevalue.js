...
sparseplacevalue.prototype.parse = function (arg) { //  2017.9
    ...
    if (typeof arg == "number") return new sparseplacevalue([[new datatype().parse(arg), new wholeplacevalue().parse(0)]]);
...
sparseplacevalue.prototype.scale = function (scalar) {
...
sparseplacevalue.prototype.pow = function (power) {        //  2016.11
    if (power instanceof this.datatype) power = new sparseplacevalue([[power, new wholeplacevalue().parse(0)]]);   //  2017.7
    if (!(power instanceof sparseplacevalue)) power = new sparseplacevalue().parse('' + power);       //  2017.7
    if (power.points.length == 1 & power.points[0][1].is0()) {//alert(JSON.stringify(power.points[0][0] instanceof rational))
        ...
        if (power.points[0][0].is0()) return new sparseplacevalue().parse(1);
        if (power.points[0][0].isneg()) return new sparseplacevalue().parse(1).divide(this.pow(new sparseplacevalue([[power.points[0][0].negate(), [new this.datatype().parse(0)]]])));
        if (power.points[0][0].isint()) return this.times(this.pow(power.sub(new sparseplacevalue().parse(1))));
    }
    if (this.points.length == 1) {  //  2017.9
        ...
        var ret = powe.points.reduce((acc, cur) => acc.times(new sparseplacevalue([cur])), new sparseplacevalue().parse(1));
        return ret;
    }
