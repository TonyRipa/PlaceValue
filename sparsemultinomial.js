...
class sparsemultinomial extends abstractpolynomial {
    ...
    constructor(base, pv) {
        ...
        if (!(pv instanceof sparseplacevaluerational)) { var s = "sparsemultinomial expects argument 2 (pv) to be a sparsePVrational not " + typeof pv; alert(s); throw new Error(s); } //  2017.7
        super();
        this.base = base
        this.pv = pv;
    }
    ...
    parse(strornode) {  //  2017.9
        ...
        if (node.type == 'SymbolNode') {
            ...
            if (node.fn == 'unaryMinus') {
                var c = new sparsemultinomial([], sparseplacevaluerational.parse(0)).sub(a);
            } else if (node.fn == 'unaryPlus') {
                var c = new sparsemultinomial([], sparseplacevaluerational.parse(0)).add(a);
            } else {
            ...
            //return new sparsemultinomial([1, null], sparseplacevaluerational.parse(Number(node.value)));
            return new sparsemultinomial([], new sparseplacevaluerational().parse(Number(node.value)));
