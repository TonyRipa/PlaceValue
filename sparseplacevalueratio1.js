...
sparseplacevalueratio1.prototype.check = function(other) {	//	2019.2	Added Type-Checker
	if (!(this instanceof sparseplacevalueratio1)) throw new Error("SparsePlaceValueRatio1.prototype.Check 1 Fail : This isn't SparsePlaceValueRatio1");
	...
	if (arguments.length === 0) {
		...
		if (numtype != dentype)
			throw new Error('SparsePlaceValueRatio1.prototype.Check 1 Fail : num is ' + translate(numtype) + ' but den is ' + translate(dentype));
		return numtype;
	}
	if (!(other instanceof sparseplacevalueratio1)) throw new Error("SparsePlaceValueRatio1.prototype.Check 2 Fail : Other isn't SparsePlaceValueRatio1");
	var othertype = other.check();
	var mytype = this.check();
	if (mytype != othertype) {
		throw new Error('SparsePlaceValueRatio1.prototype.Check 2 Fail : Othertype is of type ' + translate(othertype) + ', but ThisType is ' + translate(mytype));
	}
	return mytype;
}
...
sparseplacevalueratio1.prototype.remainder = function (den) {								//	2019.4	Added
	this.check(den);
