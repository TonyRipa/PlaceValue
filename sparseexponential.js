...
class sparseexponential extends abstractpolynomial {
    ...
    constructor(base, pv) {
        ...
        if (arguments.length < 2) pv = new sparseplacevaluerational();  //  2017.9
        ...
        if (!(pv instanceof sparseplacevaluerational)) { var s = 'sparseexponential expects argument 2 (pv) to be a sparsePVrational not ' + typeof base +':'+base; alert(s); throw new Error(s); }
        super();
        this.base = base
        this.pv = pv;
    }
    ...
    parse(strornode) {  //  2017.9
        ...
        if (node.type == 'SymbolNode') {
            ...
            var kids = node.args;
            var a = sparseexponential.parse(kids[0]);       // sparseexponential handles unpreprocessed kid   2015.11
            if (node.fn == 'unaryMinus') {
                var c = new sparseexponential([], sparseplacevaluerational.parse(0)).sub(a);
            } else if (node.fn == 'unaryPlus') {
                var c = new sparseexponential([], sparseplacevaluerational.parse(0)).add(a);
            } else {
                var b = sparseexponential.parse(kids[1]);   // sparseexponential handles unpreprocessed kid   2015.11
                var c = (node.op == '+') ? a.add(b) : (node.op == '-') ? a.sub(b) : (node.op == '*') ? a.times(b) : (node.op == '/') ? a.divide(b) : (node.op == '|') ? a.eval(b) : a.pow(b);
            }
            return c;
        } else if (node.type == 'ConstantNode') {
            //return new sparseexponential([1, null], sparseplacevaluerational.parse(Number(node.value)));
            return new sparseexponential([], new sparseplacevaluerational().parse(Number(node.value)));
            ...
            var ten = new sparseplacevaluerational().parse('1E1');   // exp is 2D    2016.1
            //var iten = sparseplacevaluerational.parse('1Ei');   // exp is 2D    2016.1
            var exp = new sparseplacevaluerational().parse('2.718').pow(kidaspoly.pv);  //  2017.5
            //alert(JSON.stringify([kidaspoly, ten, exp]));
            //var expi = sparseplacevaluerational.parse('2.718').pow(kidaspoly.pv.times(sparseplacevaluerational.parse('i')));  //  2017.5
            var exp2 = exp.pow(-1)
        ...
        function hyper(name, sign) {
            for (var b = 0; b < 3; b++) {   //  2017.5
                for (var i = 4; i >= -4; i--) {
                    ...
                    //var l = s.get([0, i]).r;
                    //var r = s.get([0, -i]).r;
                    var l = s.get(Array(b).fill(new rational().parse(0)).concat([new rational().parse(i)])).toreal();    //  2017.5
                    ...
                    if (math.sign(l) * math.sign(r) == sign && ar >= al && al != 0 && Math.abs(m) > .001) { // Math.sign to math.sign   2016.3
                        ...
                        //s = s.sub(sparseplacevaluerational.parse('1E' + '0,'.repeat(b) + i).add(sparseplacevaluerational.parse('1E' + '0,'.repeat(b) + (-i)).scale({ 'r': sign, 'i': 0 })).scale({ 'r': m, 'i': 0 }));
                        s = s.sub(new sparseplacevaluerational().parse('1E' + '0,'.repeat(b) + i).add(new sparseplacevaluerational().parse('1E' + '0,'.repeat(b) + (-i)).scale(sign)).scale(m));
                        //alert(JSON.stringify(s));
                    }
                }
            }
    ...
    //                    s = s.sub(sparseplacevaluerational.parse('1E' + '0,'.repeat(b) + '(0,' + i + ')').add(sparseplacevaluerational.parse('1E' + '0,'.repeat(b) + '(0,' + (-i) + ')').scale/({ 'r':/ sign, 'i': 0 })).scale(ind == 0 ? { 'r': m, 'i': 0 } : { 'r': 0, 'i': m }));
    //                }
    //            }
    //        }
    //        ret = ret.replace(/\+\-/g, '-');
    ...
    eval(base) {    //  2017.5
        return new this.constructor(this.base.slice(0, -1), this.pv.eval(new sparseplacevaluerational().parse('2.718').pow(base.pv)));
    }
